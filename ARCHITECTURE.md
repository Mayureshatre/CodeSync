...
- **Authentication:** NextAuth.js (Auth.js) with three providers: Credentials (email/password, bcrypt-hashed, argon2id acceptable alternative), Google OAuth, GitHub OAuth. Session strategy: JWT access token (15 min expiry) in an httpOnly, secure, `SameSite=Lax` cookie, plus a rotating refresh token (30-day expiry) in a separate httpOnly cookie, refreshed via a silent `/auth/refresh` call. Email/password accounts must verify email before `project.publish` or `application.create` are permitted (enforced in the service layer, not just a UI gate).
- **Session table:** a `Session` table (or NextAuth's adapter equivalent) stores active refresh tokens per device, surfaced in Settings → Security (SRS §12.1) with per-session revoke and "log out everywhere."
