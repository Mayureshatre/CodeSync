...
import { saveDeveloper, getSavedDevelopers } from '@/src/server/services/savedItemService';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await saveDeveloper(session.user.id, body);
...
export async function GET(req: NextRequest) {
  try {
    ...
    const results = await getSavedDevelopers(session.user.id);
